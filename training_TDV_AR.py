...
from priors import TDV, ParameterLearningWrapper
import torch
...
logging.basicConfig(
                    filename="training_TDV_AR_" + str(datetime.datetime.now()) + ".log",
                    level=logging.INFO,
                    )

only_fitting = False
if only_fitting:
    ckp = torch.load(f"weights/adversarial_{problem}/TDV_adversarial_for_{problem}.pt")
    ...
    torch.save(regularizer.state_dict(), f"weights/adversarial_{problem}/TDV_adversarial_for_{problem}.pt")
...
torch.save(wrapped_regularizer.state_dict(), f"weights/adversarial_{problem}/TDV_adversarial_for_{problem}_fitted.pt")
