...
dataset = get_dataset(
    "BSDS500_gray", test=False, transform=CenterCrop(300)
)  # RandomCrop(256))
    ...
    for epoch in range(patchnr_epochs):
        ...
        with tqdm(total=len(patchnr_dataloader)) as pbar:
            for idx, batch in enumerate(patchnr_dataloader):
                ...
                invs, jac_inv = patch_nr.normalizing_flow(
                    x
...
physics = Downsampling(
    img_size=(1, 256, 256), factor=2, device=device, noise_model=noise_model
)  # Denoising(device=device, noise_model=noise_model)
data_fidelity = L2()
...
for n_patches in [10, 100, 1000, 5000, 10000]:
    ...
    print(
        f"n_patches {patch_nr.n_patches} value of prior {patch_nr.g(image).item()} (second evaluation {patch_nr.g(image).item()})"
    )
...
def minimize_variational_problem(prior, lam, optim_steps):
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=optim_steps, eta_min=lr_variational_problem / 10.0
    )
    for i in (progress_bar := tqdm(range(optim_steps))):
        ...
        progress_bar.set_description(
            "Step {} || Loss {} || {}".format(
                i + 1, loss.item(), PSNR()(imgs, image).item()
            )
        )
    return imgs.detach()
...
total_patch_number = (image.shape[2] - patch_size + 1) * (
...
im = ax4.imshow(
    physics.A_adjoint(observation)[0, 0].detach().cpu().numpy(), cmap="gray"
)
