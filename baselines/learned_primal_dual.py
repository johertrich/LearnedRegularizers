...
class IterativeBlock(nn.Module):
    def __init__(self, n_in=3, n_out=1, n_memory=5, n_layer=3, internal_ch=32,
                 kernel_size=3, batch_norm=True, lrelu_coeff=0.2):
...
class PrimalDualNet(nn.Module):
    def __init__(self, image_width, num_angles, n_iter, op, op_adj, op_init=None, n_primal=5, n_dual=5,
                 use_sigmoid=False, n_layer=4, internal_ch=32, kernel_size=3,
                 batch_norm=True, lrelu_coeff=0.2):
        super(PrimalDualNet, self).__init__()
        self.image_width = image_width
        self.num_detector_pixels = int(np.ceil(np.sqrt(self.image_width**2 + self.image_width**2))) # this is the default for parallel beam geometry 
        ...
        for it in range(n_iter):
            self.dual_blocks.append(IterativeBlock(
                ...
                batch_norm=batch_norm, lrelu_coeff=lrelu_coeff))
            self.primal_blocks.append(
                get_unet_model(in_ch=2+self.n_primal-1, out_ch=1+self.n_primal-1, scales=4, skip=8, channels=(16,32,32,64),
                               use_sigmoid=False, use_norm=True))

    def forward(self, y):
        primal_cur = torch.zeros(y.shape[0], self.n_primal,self.image_width, self.image_width, device=y.device)

        if self.op_init is not None:
            primal_cur[:] = self.op_init(y)  # broadcast across dim=1
        dual_cur = torch.zeros(y.shape[0], self.n_dual, self.num_detector_pixels, self.num_angles, device=y.device)
