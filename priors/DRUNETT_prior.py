...
class DRUNet(nn.Module):
    ...
    def __init__(
        ...
        in_channels = in_channels  # accounts for the input noise channel
    ...
    def forward(self, x0):
        x0 = torch.nn.functional.pad(x0,(self.left_pad,self.right_pad,self.top_pad,self.bottom_pad),mode = 'reflect')
        x1 = self.m_head(x0)
        x2 = self.m_down1(x1)
        x3 = self.m_down2(x2)
        x4 = self.m_down3(x3)
        ...
        x = self.m_tail(x)
        r = x0-x
        return r[:,:,self.left_pad:-self.right_pad,self.top_pad:-self.bottom_pad]

    def regularizer(self, x0):
        x0 = torch.nn.functional.pad(x0,(self.left_pad,self.right_pad,self.top_pad,self.bottom_pad),mode = 'reflect')
        x1 = self.m_head(x0)
        x2 = self.m_down1(x1)
        x3 = self.m_down2(x2)
        ...
        x = self.m_tail(x)
        x = x0-x
        x = x[:,:,self.left_pad:-self.right_pad,self.top_pad:-self.bottom_pad]
        #print(torch.sum(torch.abs(x), dim=[1, 2, 3]))

        return 1*torch.sum(torch.abs(x)**2, dim=[1, 2, 3])#+1*torch.sum(torch.abs(x_e)**2, dim = [1,2,3])
    ...
    def compute_padding(self,image_shape):
        height, width = image_shape

        # Calculate target dimensions (next multiple of 8)
        padded_height = ((height + 7) // 8) * 8
        padded_width = ((width + 7) // 8) * 8

        # Total padding needed
        total_pad_height = padded_height - height
        total_pad_width = padded_width - width

        # Split padding equally (add extra to bottom/right if odd)
        top_pad = total_pad_height // 2
        bottom_pad = total_pad_height - top_pad
        left_pad = total_pad_width // 2
        right_pad = total_pad_width - left_pad
        self.top_pad = top_pad
        self.bottom_pad = bottom_pad
        self.left_pad = left_pad
        self.right_pad = right_pad
        
        print('padding images to size:', self.top_pad, self.bottom_pad, self.left_pad, self.right_pad)
