"""Modified deepinv.optim.optimizers"""
...
class BaseOptim(Reconstructor):
    ...
    def init_metrics_fn(self, X_init, x_gt=None):
        ...
        if x_gt is not None:
            psnr = [
                [PSNR(max_pixel=None)(x_init[i : i + 1], x_gt[i : i + 1]).cpu().item()]
    ...
    def update_metrics_fn(self, metrics, X_prev, X, x_gt=None):
        ...
        if metrics is not None:
            ...
            for i in range(self.batch_size):
                ...
                if x_gt is not None:
                    psnr = PSNR(max_pixel=None)(x[i : i + 1], x_gt[i : i + 1])
                    metrics["psnr"][i].append(psnr.cpu().item())
                    if self.verbose:
                        print(f"PSNR: {psnr.cpu().item():.4f}")
                if self.has_cost:
                    F = X["cost"][i]
