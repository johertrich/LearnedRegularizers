...
class ADMMIteration(OptimIterator):
    ...
    def forward(self, X, cur_data_fidelity, cur_prior, cur_params, y, physics):
        ...
        eff_lamb = 1 / cur_params["stepsize"]
        df = []
        for i in range(x.shape[0]):
            df.append(cur_data_fidelity(x[i : i + 1], y[i : i + 1], physics).item())
        df = torch.tensor(df)
        R = torch.tensor(R)
        obj = df + eff_lamb * R
        # print(
        #     f"df={df.mean():.3E}, R={R.mean():.3E}, lamb={eff_lamb:.2E}, df+lambda*R={obj.mean():.3E}\n"
        # )
        ...
        return {
            ...
            "lambda": eff_lamb,
            "obj": obj,
            "x_norm_inf": torch.max(torch.abs(x).reshape(x.shape[0], -1), dim=1)[
                0
            ].cpu(),
        }
...
class gStepADMM(gStep):
    ...
    def forward(self, x, z, cur_prior, cur_params):
        ...
        for i in range(out.shape[0]):
            R_out.append(0.0)
            # R_out.append(cur_prior.g(out[i : i + 1], finv_x=p[i : i + 1]).item())
        return out, R_out
