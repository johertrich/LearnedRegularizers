...
class FixedPoint(nn.Module):
    ...
    def forward(self, *args, compute_metrics=False, x_gt=None, **kwargs):
        ...
        for it in tqdm(
            ...
            self.check_conv_fn(it, X_prev, X)
            ...
            if self.check_iteration:
                ...
                for k in ["data_fidelity", "R", "obj", "x_norm_inf"]:
                    metrics_2[k].append(X.get(k, None))
        ...
        metrics.update(**{k: torch.stack(v, dim=0).T for k, v in metrics_2.items()})
        return X, metrics
