...
def reconstruct_NAG_LS(
    ...
    def f(x_in, y_in):
        return (
            data_fidelity(x_in, y_in, physics) + lmbd * regularizer.g(x_in)
        ).detach()

    def grad_f(x_in, y_in):
        return (
            data_fidelity.grad(x_in, y_in, physics) + lmbd * regularizer.grad(x_in)
        ).detach()
    ...
    def backtracking(y_in, xcurr, xold, t, taubtinit, tau, rho):
        ...
        for i in range(100):
            ...
            t_new[id_bt] = 0.5 * (
                1 + torch.sqrt(1 + 4 * tau[id_bt] * t[id_bt] ** 2 / tau_new[id_bt])
            )
            z[id_bt] = xcurr[id_bt] + (t[id_bt] - 1) / t_new[id_bt] * (
                xcurr[id_bt] - xold[id_bt]
            )
            ...
            breg_div = (
                f(xnew, y_in)
                - f(z, y_in)
                - torch.sum(grad_fz * (xnew - z), dim=(1, 2, 3))
            )
            condition_bt = (
                breg_div
                > torch.sum((xnew - z) ** 2, dim=(1, 2, 3)) / (2 * tau_new).flatten()
            )
    ...
    for step in range(NAG_max_iter):
        tau_bt_init = torch.min(
            tau[idx] / delta, torch.tensor(tau_threshold, device=x.device)
        )
        x[idx], x_old[idx], tau[idx], t[idx] = backtracking(
            y[idx], x[idx], x_old[idx], t[idx], tau_bt_init, tau[idx], rho
        )
